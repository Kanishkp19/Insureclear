...
def rl_secondary_node(state: AgentState) -> AgentState:
    refined_q  = state["refined_question"]
    domain     = state["current_domain"]
    
    # We only scan the permanent DB for secondary recommendations, not the user's uploaded temp doc
    primary_db_doc = DOMAIN_DOC_MAP.get(domain, DOMAIN_DOC_MAP["UNKNOWN"])
    
    comparisons = []
    
    # Iterate over all DB docs that are NOT the primary one selected by the router
    for d_name, doc_id in DOMAIN_DOC_MAP.items():
        if doc_id == primary_db_doc:
            continue
            
        print(f"   [RL Secondary System] Checking match for '{doc_id}'...")
        # Run RL extraction specifically on this secondary document
        resp = universal_selector.extract_payload(refined_q, [doc_id], confidence_threshold=0.0)
        
        # If it found a clause, grab the best one
        if resp.get("selected_clauses") and len(resp["selected_clauses"]) > 0:
            top_clause = resp["selected_clauses"][0]
            # Don't recommend error files
            if top_clause.get("node_id") == "Error":
                continue
                
            comparisons.append({
                "doc_id": doc_id,
                "confidence_score": top_clause.get("confidence_score", 0),
                "text": top_clause.get("text", "")
            })
            
    # Sort descending by confidence score
    comparisons.sort(key=lambda x: x["confidence_score"], reverse=True)
    
    return {"policy_comparisons": comparisons}
...
workflow.add_node("rl_secondary", rl_secondary_node)
workflow.add_node("explainer",    explainer_node)

workflow.add_edge(START,          "router")
workflow.add_edge("router",       "rl_extractor")
workflow.add_edge("rl_extractor", "rl_secondary")
workflow.add_edge("rl_secondary", "explainer")
