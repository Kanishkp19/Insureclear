...
def _clean_text(text: str) -> str:
    """
    PageIndex sometimes returns heavily corrupted text with HTML-like attributes:
    e.g. '&lt;<in 15="" a="" about="" agreeable="" ...&gt;'
    This strips those tags and attributes so the AI model sees only the core text.
    """
    if not text: return ""
    # 1. Remove anything inside &lt;<...>&gt; or <<...>>
    text = re.sub(r'&lt;.*?&gt;', ' ', text, flags=re.DOTALL)
    text = re.sub(r'<<.*?>>', ' ', text, flags=re.DOTALL)
    # 2. Remove isolated key="value" or key="" patterns
...
class UniversalInference:
    def __init__(self, model_path="./universal_selector", data_folder="./data/"):
        ...
        self.model.eval()
        print("   ✅ Universal Selector model & database loaded.")
    ...
    def extract_payload(self, question: str, targets: list,
        ...
        with torch.no_grad():
            for doc_id in resolved_ids:
                ...
                # Use clean text for the cross-encoder
                pairs  = [[question, _clean_text(n["text"])] for n in top_candidates]
